...
TEST_F(HybridFollowJointTrajectoryExecutorTest,
       SinglePointVelocityHintForwardsSampledVelocityWhenEnabled) {
    FakeRobotHw hw;
    std::mutex loop_mtx;
    auto target_config = MakeTargetConfig();
    target_config.max_velocities = {20.0, 20.0};
    ...
    ASSERT_TRUE(executor.startGoal(
        MakeGoal({"joint_a", "joint_b"}, {MakePoint({0.8, 0.4}, 0.4)}), &error))
        << error;
...
TEST_F(HybridFollowJointTrajectoryExecutorTest,
       MultiPointGoalKeepsPurePositionTrackingInSinglePointVelocityHintMode) {
    FakeRobotHw hw;
    std::mutex loop_mtx;
    auto target_config = MakeTargetConfig();
    target_config.max_velocities = {20.0, 20.0};
