    ...
    def execute(self, goal):
        ...
        while elapsed < self.exec_time:
            if self.server.is_preempt_requested():
                rospy.logwarn('[FakeCANopen] 收到 CANCEL')
        ...
        rospy.loginfo('[FakeCANopen] 执行完成, 返回 SUCCEEDED')
        self.server.set_succeeded(FollowJointTrajectoryResult(error_code=0))


if __name__ == '__main__':
    rospy.init_node('fake_canopen_arm_server')
    FakeCanopenArmServer()
    rospy.spin()
